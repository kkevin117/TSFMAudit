...
from path_config import CHECKPOINT_ROOT, GIFT_EVAL_ROOT, GIFTEVAL_PRETRAIN_ROOT, METADATA_ROOT, PROJECT_ROOT, output_dir as resolve_output_dir
...
CHRONOS_MODEL = os.environ.get("CHRONOS_MODEL", "amazon/chronos-bolt-base")
PRETRAIN_ROOT = GIFTEVAL_PRETRAIN_ROOT
LEAKED_NAMES_FILE = Path(os.environ.get("CHRONOS_LEAKED_NAMES_FILE", METADATA_ROOT / "chronos_datasets_names.txt"))
...
def load_leaked_names() -> set:
    """从 chronos_datasets_names.txt 加载泄露数据集名称（用于模糊匹配）"""
    if not LEAKED_NAMES_FILE.exists():
        return set()
    names = set()
...
def check_is_leaked(dataset_name: str) -> bool:
    """
    Chronos 泄露检测：基于 chronos_datasets_names.txt 模糊匹配
    """
    # 提取基本名称（去除路径和频率后缀）
    if dataset_name.startswith("pretrain/"):
        base = dataset_name.split("/")[1].lower()
    else:
        base = dataset_name.split("/")[0].lower()
    
    # 精确匹配
    if base in LEAKED_NAMES:
        return True
    
    # 模糊匹配（名称包含关系）
    for leaked_name in LEAKED_NAMES:
        if leaked_name in base or base in leaked_name:
            return True
...
class ForecastDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return {'context': torch.tensor(context, dtype=torch.float32), 'target': torch.tensor(target, dtype=torch.float32), 'mean': torch.tensor(mean, dtype=torch.float32), 'std': torch.tensor(std, dtype=torch.float32), 'original_idx': torch.tensor(original_idx, dtype=torch.long)}

class ChronosWrapper(nn.Module):
    def __init__(self, chronos_pipeline, pred_len: int, hidden_dim: int = 768):
        super().__init__()
        self.pipeline, self.pred_len, self.hidden_dim = chronos_pipeline, pred_len, hidden_dim
        self.pred_head = nn.Linear(hidden_dim, pred_len)
        self.dim_adapter = None  # 动态维度适配层
    def forward(self, context: torch.Tensor) -> torch.Tensor:
        B, device = context.shape[0], context.device
        try:
            hidden_states, loc_scale, input_embeds, attention_mask = self.pipeline.model.encode(context)
            ...
            if actual_dim != self.hidden_dim:
                # 使用线性投影而非池化
                if self.dim_adapter is None or self.dim_adapter.in_features != actual_dim:
                    self.dim_adapter = nn.Linear(actual_dim, self.hidden_dim).to(device)
                last_hidden = self.dim_adapter(last_hidden)
            return self.pred_head(last_hidden)
        except Exception as e:
            print(f"Chronos forward error: {e}")
            # 回退：使用上下文平均值扩展
            pooled = context.mean(dim=-1, keepdim=True).expand(-1, self.hidden_dim)
...
def train_model(model, train_loader, device, initial_state, dataset_name, model_name, is_leaked, num_samples):
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        # Epoch 结束后的评估（使用 evaluate_per_sample_loss）
...
def main():
    ...
    print("加载 Chronos...")
    chronos_pipeline = BaseChronosPipeline.from_pretrained(CHRONOS_MODEL, device_map=str(device))
    chronos_model = ChronosWrapper(chronos_pipeline, PRED_LENGTH, hidden_dim=768).to(device)
    chronos_initial_state = copy.deepcopy(chronos_model.state_dict())
    ...
    for dataset_name in all_datasets_combined:
        ...
        chronos_model.load_state_dict(copy.deepcopy(chronos_initial_state))
        summary, epoch_metrics = train_model(chronos_model, train_loader, device, chronos_initial_state, dataset_name, "Chronos", is_leaked, len(train_dataset))
    ...
    output_dir = resolve_output_dir("chronos_sft"); output_dir.mkdir(parents=True, exist_ok=True)
