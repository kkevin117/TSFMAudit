...
from path_config import GIFT_EVAL_ROOT, GIFTEVAL_PRETRAIN_ROOT, PROJECT_ROOT, optional_repo_path, output_dir as resolve_output_dir

sys.path.insert(0, str(PROJECT_ROOT / "src"))
KAIROS_REPO = optional_repo_path("KAIROS_REPO", ["Kairos"])
if KAIROS_REPO is not None:
    sys.path.insert(0, str(KAIROS_REPO))
...
# 统一配置
# ============================================================
KAIROS_MODEL = os.environ.get("KAIROS_MODEL", "mldi-lab/Kairos_50m")
...
# 模型包装器
# ============================================================
class KairosWrapper(nn.Module):
    """Kairos 模型包装器（用于微调）"""
    
    def __init__(self, kairos_model, pred_len: int, context_len: int, hidden_dim: Optional[int] = None):
        super().__init__()
        self.kairos = kairos_model
        self.pred_len = pred_len
        self.context_len = context_len
        
        # Kairos 模型的隐层维度，尝试从 config 获取
        if hidden_dim is not None:
            self.hidden_dim = hidden_dim
        elif hasattr(kairos_model, 'config') and hasattr(kairos_model.config, 'hidden_size'):
            self.hidden_dim = kairos_model.config.hidden_size
        else:
            self.hidden_dim = 512  # 默认值
    ...
    def forward(self, context: torch.Tensor) -> torch.Tensor:
        ...
        try:
            # 创建 attention mask（非 NaN 位置为 True）
            mask = ~torch.isnan(context)
            context_clean = torch.nan_to_num(context, nan=0.0)
            
            # 通过 encoder 获取表示
            encoder_outputs = self.kairos.encode(
                context=context_clean,
                mask=mask,
                output_attentions=False,
            )
            
            # encoder_outputs 是 tuple，第一个元素是 hidden_states
            if isinstance(encoder_outputs, tuple) and len(encoder_outputs) >= 1:
                hidden_states = encoder_outputs[0]  # [B, num_patches, hidden_dim]
            else:
...
def main():
    ...
    try:
        ...
        from kairos_pipeline.model import AutoModel
    
    kairos_base = AutoModel.from_pretrained(KAIROS_MODEL, trust_remote_code=True)
    kairos_base = kairos_base.to(device)
    kairos_model = KairosWrapper(kairos_base, PRED_LENGTH, CONTEXT_LENGTH).to(device)
    kairos_initial_state = copy.deepcopy(kairos_model.state_dict())
    ...
    for dataset_name in all_datasets_combined:
        ...
        kairos_model.load_state_dict(copy.deepcopy(kairos_initial_state))
        summary, epoch_metrics = train_model(
            kairos_model, train_loader, device, kairos_initial_state,
            dataset_name, "Kairos", is_leaked, len(train_dataset)
        )
    ...
    # 保存结果
    output_dir = resolve_output_dir("kairos_sft")
