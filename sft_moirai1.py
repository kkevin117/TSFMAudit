...
from path_config import DATA_ROOT, GIFT_EVAL_ROOT, GIFTEVAL_PRETRAIN_ROOT, PROJECT_ROOT, output_dir as resolve_output_dir
...
# 统一配置
# ============================================================
MOIRAI_MODEL = os.environ.get("MOIRAI_MODEL", "Salesforce/moirai-1.0-R-small")
...
LOTSA_DATA_ROOT = Path(os.environ.get("LOTSA_DATA_ROOT", DATA_ROOT / "LOTSA"))


def check_is_leaked(dataset_name: str) -> bool:
    """根据 lotsa_data 目录检查数据集是否泄露"""
    if not LOTSA_DATA_ROOT.exists():
        return False
    ...
    dataset_path = LOTSA_DATA_ROOT / base_name
...
class MoiraiWrapper(nn.Module):
    ...
    def __init__(self, moirai_module, pred_len: int, context_len: int, patch_size: int = 32):
        super().__init__()
        self.module = moirai_module
        self.pred_len = pred_len
        self.context_len = context_len
        self.patch_size = patch_size
        
        # 获取 encoder 输出维度并添加预测头
        hidden_dim = 384  # Moirai-small 的隐层维度
    ...
    def forward(self, context: torch.Tensor) -> torch.Tensor:
        ...
        # 添加通道维度：[B, 1, context_len]
        x = context.unsqueeze(1)
        
        # 创建 patch 输入
        num_patches = self.context_len // self.patch_size
        x_patches = x.view(B, 1, num_patches, self.patch_size)
        
        # 通过 encoder 获取表示
        try:
            hidden = self.module.encoder(
                target=x_patches.squeeze(1),  # [B, num_patches, patch_size]
                observed_mask=torch.ones_like(x_patches.squeeze(1)),
                sample_id=torch.zeros(B, dtype=torch.long, device=device),
                time_id=torch.arange(num_patches, device=device).unsqueeze(0).expand(B, -1),
                variate_id=torch.zeros(B, num_patches, dtype=torch.long, device=device),
                prediction_mask=torch.zeros(B, num_patches, dtype=torch.bool, device=device),
            )
            ...
            hidden = F.adaptive_avg_pool1d(x, 384).squeeze(1)  # [B, 384]
...
def main():
    ...
    print("加载 Moirai1...")
    moirai_module = MoiraiModule.from_pretrained(MOIRAI_MODEL)
    moirai_model = MoiraiWrapper(moirai_module, PRED_LENGTH, CONTEXT_LENGTH, PATCH_SIZE).to(device)
    moirai_initial_state = copy.deepcopy(moirai_model.state_dict())
    ...
    for dataset_name in all_datasets_combined:
        ...
        moirai_model.load_state_dict(copy.deepcopy(moirai_initial_state))
        summary, epoch_metrics = train_model(
            moirai_model, train_loader, device, moirai_initial_state,
            dataset_name, "Moirai1", is_leaked, len(train_dataset)
        )
    ...
    # 保存结果
    output_dir = resolve_output_dir("moirai1_sft")
